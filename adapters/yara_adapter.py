...
def parse_yara_report(report_path):
    """Parse YARA output into myESI unified schema."""
    findings = []
    
    # Check if file exists to prevent crashes
    if not os.path.exists(report_path):
        print(f"[Adapter] ⚠️ YARA report not found at {report_path}")
        return findings
    ...
    for line in lines:
        ...
        if len(parts) >= 2:
            ...
            findings.append({
                ...
                "description": f"The binary matches the YARA rule '{rule_name}'. This indicates a known malware family, suspicious pattern, or specific packer.",
                "location": f"File: {file_path}",
                "remediation_guidance": "Investigate the matched rule. If this is a known malware family or malicious pattern, isolate the file and perform deep dynamic analysis.",
                "source": "YARA",
