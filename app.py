...
def run_scan_async(scan_id, target_type, target_value, client_name, file_path=None):
    try:
        ...
        findings = []
        if target_type == 'url':
            findings = scan_web_target(target_value)
...
def generate_report(scan_id):
    ...
    # Use the REAL findings generated by the adapters
    ...
    engines_map = {
        'url': 'Nikto (Dynamic CLI)',
        'mobile': 'Apktool + Grep (Dynamic Analysis)',
        'windows': 'pefile + strings (Dynamic Analysis)',
        'linux': 'dpkg-deb + checksec (Dynamic Analysis)'
    }
